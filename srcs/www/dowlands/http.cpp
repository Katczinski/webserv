...
std::string pemennaya_ot_Dimbl = "/local/index.php";
std::string methods = "GET, DELETE, POST";
// 413 -- размере requeeset Больше лимита(3к вроде)
// 
...
bool request(ft::Response& req, int i)
{
    time_t now = time(0);
...
bool general_header_check(std::vector<std::string> header, ft::Response& req)
{
...
void ft_post(ft::Response& req)
{
    // std::cout << "Content-Type " << req.full_log["Content-Type"] << std::endl;
    // std::cout << "Request " << req.full_log["Body"] << std::endl;
}
int main()
{
    ft::Response req;
    ...
    // чтение файла
    std::ifstream file("test.txt"); // тут скорее всего будет чтение их FD от клиента, но я ХЗ на самом деле от Вовы крч FD
    std::getline(file, buffer, '\n');
    ft_split(buffer, ' ', splited_words); // проверяем первую строку
    if(general_header_check(splited_words, req))
    {
